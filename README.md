...
**(1)** insertNode <some unsigned int data>

The server will insert the node with the specified data in the tree. Note that this operation
modifies the BST.
...
**(2)** deleteNode <some unsigned int data>

The server will delete the node with the specified data from the tree. Note that this
...
Thus, for Client1 the server will first insert a node with data 19675, then perform the
...
designed. If these are different from one another or different from the expected data it will
